...
int main(int argc, char** argv)
{
    ...
    */
    ...
    seam_halfedge_descriptor smhd = seam_mesh.add_seams("git_repos/Confined_active_particles/meshes/cut_line.selection.txt");
    ...
    std::cout << "the seam mesh has: " << num_halfedges(seam_mesh) << " halfedges" << std::endl;
    // for(halfedge_descriptor hd : halfedges(sm)){
    //     std::cout << hd << " ";
    // }
    ...
    // typedef SMP::Circular_border_arc_length_parameterizer_3<Seam_mesh> Border_parameterizer;
    typedef SMP::Square_border_uniform_parameterizer_3<Seam_mesh> Border_parameterizer;
    ...
    typedef SMP::Iterative_authalic_parameterizer_3<Seam_mesh, Border_parameterizer> Parameterizer;
    Parameterizer parameterizer(border_parameterizer);

    const unsigned int iterations = (argc > 2) ? std::atoi(argv[2]) : 15;
    SMP::Error_code err = parameterizer.parameterize(seam_mesh, bhd, uv_map, iterations);
