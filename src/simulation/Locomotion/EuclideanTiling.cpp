...
EuclideanTiling::EuclideanTiling(
    ...
    Eigen::VectorXi& n)
    : surface_parametrization(surface_parametrization), tessellation(tessellation), r_UV(r_UV), r_UV_old(r_UV_old), n(n)
{
}
...
std::pair<std::string, Point_2_eigen> EuclideanTiling::check_border_crossings(
    const Point_2_eigen& start_eigen, const Point_2_eigen& end_eigen)
{
...
std::tuple<Eigen::Vector2d, double, Eigen::Vector2d> EuclideanTiling::processPoints(
    const Eigen::Vector2d& pointA, const Eigen::Vector2d& point_outside, double n)
{
...
bool EuclideanTiling::is_point_on_segment(const Point_2_eigen& P, const Point_2_eigen& A, const Point_2_eigen& B)
{
    ...
    if (P.x() < std::min(A.x(), B.x()) || P.x() > std::max(A.x(), B.x()) || P.y() < std::min(A.y(), B.y())
        || P.y() > std::max(A.y(), B.y()))
    {
        return false;
    }
...
std::optional<Point_2_eigen> EuclideanTiling::intersection_point(
    const Segment_2_eigen& line, const std::vector<Point_2_eigen>& border)
{
